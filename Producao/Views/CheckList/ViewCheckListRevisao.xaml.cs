...
public class SiglaColorConverter : IValueConverter
{
    public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
    {
        if (value is not ControleMemorialModel data)
            return DependencyProperty.UnsetValue;

        if (data.altera_ok?.Trim().Contains('0') == true &&
            data.memorial_alterado_por != null)
        {
            return new SolidColorBrush(Colors.Yellow);
        }

        if (data.memorial_data_alterado.HasValue &&
            data.confirma_alteracao_data.HasValue &&
            data.memorial_data_alterado.Value.Date >
            data.confirma_alteracao_data.Value.Date)
        {
